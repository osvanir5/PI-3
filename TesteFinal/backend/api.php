...
switch ($action) {
    ...
    case 'get_results':
        $election_id = $_GET['election_id'] ?? 0;
        $stmt = $pdo->prepare("SELECT title, is_finished FROM elections WHERE id = ?");
        $stmt->execute([$election_id]);
        $election = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        echo json_encode(['success' => true, 'title' => $election['title'], 'is_finished' => $election['is_finished'], 'results' => $results]);
        break;
